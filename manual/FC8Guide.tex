...
\Huge \FC~8~~~User's Guide}}

\hfill\copy\titlebox

\vspace*{1ex}

\hfill\hbox{February 16, 2015~~~~~Thomas Hahn}
...
\Code{GaugeTerms} & \Code{True} &
	whether to drop the $\eta$-dependent terms \\
\Code{MomElim} & (taken from \Code{CalcFeynAmp}) &
	how to apply momentum conservation \\
\Code{DotExpand} & (taken from \Code{CalcFeynAmp}) &
	whether to expand terms collected for momentum elimination \\
...
\Code{GaugeTerms} drops terms containing the gauge-dependent auxiliary 
...
invariants (but no four-vectors) by squaring the amplitude and 
computing the polarization sums, as outlined above.  This has the 
advantage of being independent of the representation of the spinors and 
vectors, but of course the size of the expression is roughly doubled by 
squaring.
...
\Code{TagCollect[\Var{expr},\,\Var{tag},\,\Var{f}]} &
	apply \Var{f} to the part of \Var{expr} tagged by \Var{tag} \\
...
\Code{ApplyUnitarity[\Var{expr},\,\Var{U},\,\Vd,\,\Vs]} &
	simplify expr by exploiting the unitarity of the \Vd-dimensional 
	matrix \Var{U}.  The optional argument \Vs\ specifies the 
	simplification function to use internally (default: 
	\Code{FullSimplify}) \\
...
For example,
\begin{verbatim}
  OnSize[100, FullSimplify, 500, Simplify, DenCollect]
\end{verbatim}
...
\Code{ApplyUnitarity} uses unitarity of a given matrix to simplify an
expression.  The two unitarity relations $UU^\dagger = \unity$ or
...
\quad\Code{xsection.F} \>
	the code for computing the cross-section \\
\quad\Code{xsection.h} \>
	declarations for \Code{xsection.F} \\
\quad\Code{process.h} \>
	the process specification \\
\quad\Code{run.F} \>
	the parameters for a particular ``run'' \\
\quad\Code{model\uscore sm.F,.h} \>
	initialization of the Standard Model parameters \\
\quad\Code{model\uscore mssm.F,.h} \>
	initialization of the MSSM parameters \\
\quad\Code{model\uscore thdm.F,.h} \>
	initialization of the two-Higgs-doublet-model parameters \\
\quad\Code{lumi\uscore parton.F} \>
	luminosity computation for a partonic process \\
\quad\Code{lumi\uscore hadron.F} \>
	luminosity computation for a hadronic process \\
\quad\Code{lumi\uscore photon.F} \>
	luminosity computation for a photonic process \\
\quad\MtoN.F \>
	the kinematics for a $\Var{M}\to\Var{N}$ process \\
\quad\MtoN.h \>
	declarations for \MtoN.F \\
\quad\Code{configure} \>
...
\Code{FileHeader} & \Code{"\#if 0\backsl n} &
	...
	& \rlap{\Code{* generated by FormCalc 8.4 \%t\backsl n}} \\[-\parskip]
...
\Line(55,77)(55,62)
\Line(55,62)(65,62)
\Text(70,63)[l]{\Code{squaredme/}}
\Text(70,52)[l]{\small (generated by \Code{WriteSquaredME})}
%
\Line(45,77)(45,37)
\Line(45,37)(65,37)
\Text(70,38)[l]{\Code{renconst/}\hphantom{p}}
\Text(70,27)[l]{\small (generated by \Code{WriteRenConst})}
%
\Line(35,77)(35,12)
\Line(35,12)(65,12)
\Text(70,13)[l]{\Code{util/}\hphantom{p}}
\Text(70,2)[l]{\small (comes with \FC)\hphantom{p}}
\end{picture}
\end{center}
Each folder is equipped with its own makefile which makes a library of 
the same name, \eg the makefile in \Code{util/} makes the library 
\Code{util.a}.
...
\end{verbatim}
The master makefile is no longer overwritten in the code-generation 
process and is treated like the other driver programs, \ie a customized
copy can be saved in the local drivers directory.%
\indextt{makefile}%
\indextt{LIBS}

Occasionally it is useful to have more than one instance of
\Code{squaredme} (or \Code{renconst}), \eg when computing an
hadronic cross-section to which several partonic processes contribute. 
Both \Code{WriteSquaredME} and \Code{WriteRenConst} have the
\Code{Folder} option, with which a unique folder name can be chosen, and
the \Code{SymbolPrefix} option, with which the symbols visible to the
linker can be prefixed with a unique identifier.  This identifier is
inserted only at compile time and can easily be changed in the
sub-makefile at any time.
...
\end{itemize}
Older versions of \FC\ used to include these directly into the main
program.  The library version has the advantage that the linker selects
only the routines that are actually needed, and furthermore it is
straightforward to add new code.  The \Code{util.a} library is
compiled once when \FC\ is installed and then copied to the main code
