...
#do i = 1, `Legs'
#ifdef `k`i''
b `Patterns', addM, mulM, Den, GA, Spinor,
  intM, cutM, i_, SumOver, `SUNObjs';
.sort
...
id Spinor([p1]?, ?a) * GA(?g) * Spinor([p2]?, ?b) =
  CH(Spinor([p1], ?a), ?g, Spinor([p2], ?b)) `subst';
...
while( count(ORD, 1) );
  ...
  repeat;
    id NOW([p1]?, [p2]?) * CH(Spinor([p1]?, [m1]?, [s1]?), ?g,
                              Spinor([p2]?, [m2]?, [s2]?)) =
      CH(Spinor([p1], [m1], [s1]), ?g, 
         Spinor([p2], [m2], [s2]));
...
cf TMP, MOM, ABB, ORD, NOW, CH;
t NUM, EQ, NEQ, EPS(antisymm), DD;
nt GA, GB, GC, GD;
f WC;
auto s ARG;
set CUTINT: `CutInt';
...
id intM(Den([p1]?, [m1]?)) = replace_(q1, 2*q1 - [p1]) * intM(1, [m1]);
#do n = 2, {2*`Legs'}
also intM(<Den([p1]?, [m1]?)>,...,<Den([p`n']?, [m`n']?)>) =
  replace_(q1, 2*q1 - [p1]) *
    intM(`n', <[p2]-[p1]>,...,<[p`n']-[p1]>, <[m1]>,...,<[m`n']>);
#enddo
...
if( count(cutM, 1) == 0 ) totensor q1, NUM;

*----------------------------------------------------------------------

b NUM, Den, intM, `Tensors', CUTRAT;
.sort
...
id NUM([mu]?, [nu]?, [ro]?, [si]?) * intM(4, ?p) =
  NUM([mu], [nu], [ro], [si]) * intM(4, ?p) -
...
id NUM([mu]?, [nu]?, [ro]?) * intM(3, [p1]?, [p2]?, ?m) =
  NUM([mu], [nu], [ro]) * intM(3, [p1], [p2], ?m) +
...
#define P(m,n,x) "<[i`~m']`~x'>,...,<[i`~n']`~x'>"
#define Tag(n) "TMP(<paveM(1)*[i1]> + ... + <paveM(`~n')*[i`~n']>)"

id intM(1, ?m) = intM( A0i(0),
  ?m );

id intM(2, `P(1,1,?)', ?m) = `Tag(1)' * intM( B0i(0),
  MOM([i1] - 0),
  ?m );

id intM(3, `P(1,2,?)', ?m) = `Tag(2)' * intM( C0i(0),
  MOM([i1] - 0), MOM([i2] - [i1]), MOM(0 - [i2]),
  ?m );

id intM(4, `P(1,3,?)', ?m) = `Tag(3)' * intM( D0i(0),
  MOM([i1] - 0), MOM([i2] - [i1]), MOM([i3] - [i2]), MOM(0 - [i3]),
  MOM([i2] - 0), MOM([i3] - [i1]),
  ?m );

id intM(5, `P(1,4,?)', ?m) = `Tag(4)' * intM( E0i(0),
  MOM([i1] - 0), MOM([i2] - [i1]), MOM([i3] - [i2]), MOM([i4] - [i3]), MOM(0 - [i4]),
  MOM([i2] - 0), MOM([i3] - [i1]), MOM([i4] - [i2]), MOM(0 - [i3]), MOM([i1] - [i4]),
  ?m );

id intM(6, `P(1,5,?)', ?m) = `Tag(5)' * intM( F0i(0),
  MOM([i1] - 0), MOM([i2] - [i1]), MOM([i3] - [i2]), MOM([i4] - [i3]), MOM([i5] - [i4]), MOM(0 - [i5]),
  MOM([i2] - 0), MOM([i3] - [i1]), MOM([i4] - [i2]), MOM([i5] - [i3]), MOM(0 - [i4]), MOM([i1] - [i5]),
  MOM([i3] - 0), MOM([i4] - [i1]), MOM([i5] - [i2]),
  ?m );
...
repeat id TMP([p1]?) * NUM([mu]?, ?i) = TMP([p1]) * d_([p1], [mu]) * NUM(?i);
...
id Dminus4 * cutM(?a) = Dminus4 * cutM(?a);
...
#call FierzPre(*ORD([p1])*ORD([p2]), chainin ORD;)
#call FierzOrdered
#call DiracFinal
#break
...
#endif
#enddo
#call FierzPre(, multiply ORD(`order');)
#call FierzOrdered
#call DiracFinal
...
b cutM, qfM, Dminus4, SumOver, Mat, Den;
.sort

collect FormSimplify;

b cutM, qfM, Dminus4, SumOver, Mat;
.sort
...
*id cutM([n]?, ?a) * numM([x]?) = CUTINT[[n]](
*  numM([x] * replace_(Dminus4, 0)),
*  numM([x] * (replace_(Dminus4, 1) - replace_(Dminus4, 0))),
*  ?a );
