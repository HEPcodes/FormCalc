...
static void report_error(MLCONST char *tag, const char *arg)
{
  int p;
...
static char *getabbr(char *s)
{
  ...
  p = strlen(s);
  Allocate(lp, p + strlen(mmares) + 2 + sizeof(BTREE));
  lp->lt = lp->gt = NULL;
  *node = lp;
  strcpy(lp->abb, s);
  s = lp->sym = lp->abb + p + 1;
...
static TERM *downsize(TERM *tp, char *end)
{
  TERM *new;
  char **f;
  int off;
...
static char *putfac(char *to, char *from)
{
  ...
  do {
    for( old = termp; (tp = old->last); ) {
      ...
      if( termp->coll == 0 ) {
        ...
        Allocate(termp->f[LEVEL_PAVE], maxintsize);
        s = putfac(termp->f[LEVEL_PAVE], s);
        termp->coll = 1;
      }
      *(s - 1) = '+';
...
static void orderchain(TERM *t1p, int level)
{
  TERM *t2p, *old2, *ini;
...
nextline:
    for( ; ; ) {
      ...
      if( inexpr ) {
        if( line[0] != '\n' ) break;
        if( di == (char *)tp + sizeof(TERM) ) continue;
        *di++ = 0;
        termp = downsize(termp, di);
      }
      else if( (er = strchr(line, '=')) == NULL ) continue;
